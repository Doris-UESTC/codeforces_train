...
class Segment_tree{
    ...
    void pushdown(int node){
        ...
        tree[node<<1].val=tree[node<<1].val|tree[node].lazy;
        tree[(node<<1)+1].val=tree[(node<<1)+1].val|tree[node].lazy;
        tree[node].lazy=0;
    }

    int query(int node, int l,int r, int st ,int sp){  
        if(st>r || sp<l) return LLONG_MAX;    
    ...
    void update(int node, int l,int r, int st , int sp,int value){
        ...
        if(st>=l && sp<=r){
            ...
            tree[node].val=tree[node].val|value;
            return;
        }
        else{
            pushdown(node);
